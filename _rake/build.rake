...
task :deploy_github => :build do |t, args|
  ...
  if git_requires_attention("gh_pages")
...
task :create_post, [:date, :title, :category, :content] do |t, args|
  ...
  if args.date != nil and args.date != 'nil' and args.date != '' and args.date.match(/[0-9]+-[0-9]+-[0-9]+/) == nil
  ...
  if post_category and Dir.exists?(File.join(post_category, $post_dir))
  ...
  if not File.exists?(post_dir + filename)
...
def file_change_ext(filename, newext)
  if File.extname(filename) == ".textile" or File.extname(filename) == ".md"
